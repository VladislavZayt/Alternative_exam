...
class Parser(object):
    def __init__(self, dataset):
        ...
        self.tok2id_inv = {v: k for (k, v) in tok2id.items()}
    ...
    def vectorize(self, examples):
        ...
        return vec_examples
    def devectorize(self, vec_example):
        words = [self.tok2id_inv[w] if w in self.tok2id_inv else self.UNK for w in vec_example['word']]  # Пропускаем ROOT
        return words
     
    def read_str(self, input_string):
        nlp = spacy.load("en_core_web_sm")
        doc = nlp(input_string)
        pos_tags = [token.pos_ for token in doc]
    
        string_dict = {'word' : input_string.lower().split()}
        string_pos = {'pos': pos_tags}
        word = [self.ROOT] + [self.tok2id[w] if w in self.tok2id
                                    else self.UNK for w in string_dict['word']]
        
        pos = [self.P_ROOT] + [self.tok2id[P_PREFIX + w] if P_PREFIX + w in self.tok2id
                                   else self.P_UNK for w in string_pos['pos']]
